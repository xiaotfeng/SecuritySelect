...
class LabelPool(object):
    ...
    def hs300_industry_weight(self,
        ...
        # industry weight
        ind_weight = data_.groupby('industry',
                                   group_keys=False).apply(lambda x: x.groupby(as_index=True,
                                                                               level=K.TRADE_DATE.value).sum())

        # fill weight and industry
        res_simple = pd.merge(ind_weight.reset_index(), industry.reset_index(), on=['date', 'industry'], how='right')
        index_ = industry_exposure.index.get_level_values('date').drop_duplicates()
        df_weight_ = res_simple.pivot_table(index='date', columns='stock_id', values='weight')
        # df_industry_ = res_simple.pivot_table(index='date', columns='stock_id', values='industry')

        df_weight = df_weight_.reindex(index_).fillna(method='ffill').stack()
        # df_industry = df_industry_.reindex(index_).fillna(method='ffill').stack()

        # res = pd.concat([df_weight, df_industry], axis=1)
        # res.columns = ['weight', 'industry']
    ...
    def merge_labels(self, **kwargs) -> pd.DataFrame:
        ...
        for values_ in kwargs.values():
            container.append(values_)

        res = pd.concat(container, axis=1)
